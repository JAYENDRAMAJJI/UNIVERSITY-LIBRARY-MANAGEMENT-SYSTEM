...
export interface PrintMemberCardOptions {
  isReprint?: boolean;
...
/**
 * Standardized High-Fidelity Print & Reprint Engine for Student / Member Library Passes
 * Generates ISO/IEC CR80 Dual-Sided Badges with Crisp 1D Code 128 Barcode & 2D QR Code.
 */
export function printMemberLibraryCard(member: any, options?: PrintMemberCardOptions) {
  if (!member || typeof window === 'undefined') return;
  const printWindow = window.open('', '_blank', 'width=850,height=750');
  if (!printWindow) return;

  const cardNo = member.memberCardNo || member.barcode || 'STU-2026-0001';
  const qrSvg = generateQrSvgString(cardNo, 75);
  const barcodeSvg = generateBarcodeSvgString(cardNo, { height: 46 });
  const isReprint = Boolean(options?.isReprint);
  const issuedDate = options?.issuedDate || member.approvedDate || member.registeredDate || new Date().toISOString().split('T')[0];
  const expiryDate = options?.expiryDate || 'DEC 2028';
  const roleTitle = member.role || 'STUDENT';
  const avatarUrl = member.avatarUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80';

  const html = `
    <!DOCTYPE html>
    <html>
      <head>
        <title>Print Digital Library Pass - ${member.name} (${cardNo})</title>
        <style>
          ...
          * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          ...
          .reprint-badge { display: inline-block; background: #f59e0b; color: #000000; font-size: 8px; font-weight: 900; padding: 1px 6px; border-radius: 4px; text-transform: uppercase; margin-left: 6px; }
        </style>
      </head>
      <body>
        <div class="no-print page-title">
          <button onclick="window.print()" class="print-btn">🖨️ ${isReprint ? 'Reprint Digital Library Pass (Official Copy)' : 'Print Digital Library Pass (Front & Back)'}</button>
        </div>
        ...
        <div class="cards-container">
          ...
          <div class="id-card">
            ...
            <div class="card-body-front">
              <img src="${avatarUrl}" class="avatar-photo" alt="${member.name}" />
              <div class="member-details">
                <h3 class="member-name">${member.name}</h3>
                <div class="card-no">${cardNo}</div>
                <div class="dept-text">Dept: ${member.department || 'General'}</div>
                <div class="status-pill">● ACTIVE MEMBER ${isReprint ? '<span class="reprint-badge">REPRINT</span>' : ''}</div>
              </div>
            ...
            <div class="card-footer">
              ...
              <span>Valid Through: ${expiryDate}</span>
          ...
          <div class="id-card">
            ...
            <div class="rules-notice">
              ...
              ${isReprint ? `<br/><strong style="color: #f59e0b;">Official Replacement Pass (${options?.reprintReason || 'System Reissuance'})</strong>` : ''}
            </div>
