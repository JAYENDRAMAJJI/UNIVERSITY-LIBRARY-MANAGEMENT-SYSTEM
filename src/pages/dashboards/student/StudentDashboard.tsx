    ...
    <div className="space-y-5 sm:space-y-7 pb-10">
      ...
      <div className="bg-gradient-to-r from-slate-950 via-indigo-950 to-blue-900 rounded-3xl p-5 sm:p-7 lg:p-8 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-5 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-2">
          ...
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold font-poppins tracking-tight">Welcome back, {studentDisplayName}!</h1>
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs font-medium text-slate-300 pt-0.5">
        ...
        <div className="relative z-10 flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-2xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-slate-950 font-extrabold text-xs shadow-md transition-all flex items-center gap-2 cursor-pointer active:scale-95"
          >
      ...
      {/* Student Digital Library Pass Modal */}
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-100 space-y-5 animate-scaleUp">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-slate-900 font-poppins">Official Student Library Pass</h3>
              </div>
              <button
                type="button"
                onClick={() => setIsLibraryPassModalOpen(false)}
            ...
            {/* Smart Dual-Sided Card Box */}
            <div className="w-full rounded-2xl p-5 bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 text-white shadow-xl border-2 border-white/20 relative overflow-hidden flex flex-col justify-between min-h-[220px]">
              {passSide === 'front' ? (
                /* FRONT */
                <>
                  <div className="flex items-center justify-between border-b border-white/10 pb-2 mb-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] font-extrabold tracking-wider text-blue-300 uppercase font-poppins">UNIVERSITY CENTRAL LIBRARY</span>
                    </div>
                    <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-400/30 text-[8.5px] font-extrabold text-emerald-300 uppercase">ACTIVE</span>
                  </div>

                  <div className="flex items-center gap-3 py-1">
                    <img
                      src={studentMember.avatarUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80'}
                      alt={studentMember.name}
                      className="w-14 h-14 rounded-xl object-cover border-2 border-amber-400/60 shadow-md shrink-0"
                    />
                    <div className="min-w-0 flex-1 space-y-0.5">
                      <span className="text-[8.5px] font-bold px-1.5 py-0.5 rounded bg-blue-500/30 text-blue-200 uppercase">STUDENT PASS</span>
                      <h4 className="text-sm font-extrabold font-poppins truncate text-white">{studentDisplayName}</h4>
                      <div className="font-mono text-xs font-bold text-amber-300">{studentMember.memberCardNo}</div>
                      <p className="text-[9px] text-slate-300 truncate">Dept: {studentMember.department}</p>
                    </div>
                    <div
                      className="bg-white p-1 rounded-xl shadow shrink-0 w-14 h-14 flex items-center justify-center overflow-hidden"
                      dangerouslySetInnerHTML={{
                        __html: generateQrSvgString(studentMember.memberCardNo, 56),
                      }}
                    />
                  </div>

                  <div className="flex items-center justify-between border-t border-white/10 pt-1.5 mt-2 text-[9px] text-slate-400 font-mono">
                    <span>Valid: DEC 2028</span>
                    <span className="text-amber-300 font-bold">SECURITY VERIFIED</span>
                  </div>
                </>
              ) : (
                /* BACK */
                <>
                  <div className="flex items-center justify-between border-b border-white/10 pb-2 mb-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] font-extrabold text-amber-300 uppercase font-poppins">BARCODE & TURNSTILES</span>
                    </div>
                    <span className="font-mono text-[10px] font-bold text-slate-300">{studentMember.memberCardNo}</span>
                  </div>

                  <div className="bg-white p-2.5 rounded-xl shadow my-1 flex flex-col items-center justify-center">
                    <div
                      className="w-full flex justify-center"
                      dangerouslySetInnerHTML={{
                        __html: generateBarcodeSvgString(studentMember.memberCardNo, { height: 42 }),
                      }}
                    />
                    <p className="text-[9px] font-mono font-extrabold text-slate-900 tracking-widest mt-0.5">{studentMember.memberCardNo}</p>
                  </div>

                  <p className="text-[8.5px] text-slate-400 border-t border-white/10 pt-1 text-center">
                    Present card at library turnstiles, attendance gates, and circulation desks.
                  </p>
                </>
              )}
            </div>
            ...
            <div className="flex items-center justify-between gap-2 pt-1">
              <button
                type="button"
                onClick={() => setPassSide(passSide === 'front' ? 'back' : 'front')}
              ...
              >
                <RotateCw className="w-3.5 h-3.5 text-blue-600" /> Flip to {passSide === 'front' ? 'Back (Barcode)' : 'Front (QR)'}
              </button>

              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                  onClick={() => printMemberLibraryCard(studentMember, { isReprint: false })}
                ...
                <button
                  type="button"
                  onClick={() => printMemberLibraryCard(studentMember, { isReprint: true })}
                  className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-900 text-amber-300 border border-amber-400/30 font-bold text-xs shadow-xs transition-all flex items-center gap-1 cursor-pointer"
                >
                  Reprint
                </button>
              </div>
