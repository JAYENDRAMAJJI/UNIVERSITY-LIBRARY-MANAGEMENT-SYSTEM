                        ...
                        <td className="py-3.5 px-4 font-mono font-bold">
                          {(() => {
                            ...
                            const extensionReq = (storeState.extensionRequests || []).find(
                              (r) => (r.transactionId === tx.id || r.accessionNo === tx.accessionNo) && r.status === 'APPROVED'
                            );
                            const rejectedExtReq = (storeState.extensionRequests || []).find(
                              (r) => (r.transactionId === tx.id || r.accessionNo === tx.accessionNo) && r.status === 'REJECTED'
                            );
                            const hasExtension = Boolean(extensionReq);
                            const hasRenewal = (tx.renewalCount || 0) > 0;

                            if (fineInfo.fineAmount <= 0) {
                              return (
                                <div className="inline-flex flex-col items-center">
                                  <span className="text-slate-600 font-medium text-xs">₹0.00</span>
                                  {hasExtension ? (
                                    <span
