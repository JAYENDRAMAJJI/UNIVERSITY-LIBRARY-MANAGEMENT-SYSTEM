...
import { BookOpen, Menu, X, User as UserIcon, LogOut, Bell, CheckCircle2 } from 'lucide-react';
...
export default function Navbar({ onToggleMobileSidebar }: NavbarProps) {
  ...
  const notices: Notice[] = (state.notices || []).filter((notice) => {
    // 1. If notification is targeted to a specific email/recipient
    ...
    // 2. If notification has targetAudience specified
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 h-full">
        <div className="flex h-full justify-between items-center gap-4">
          ...
          <div className="hidden lg:flex items-center gap-1 xl:gap-2">
            ...
            {/* Notification & Logout / Login Buttons */}
            <div className="flex items-center gap-2">
              ...
              {user ? (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs xl:text-sm font-semibold text-slate-700 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 shadow-2xs transition-all whitespace-nowrap cursor-pointer"
                >
            ...
            {user ? (
              <>
                <Link
                  to={getDashboardLink()}
                  onClick={() => setIsOpen(false)}
                  className="block rounded-xl px-4 py-2.5 text-sm font-bold text-blue-700 bg-blue-50"
                >
                  {user.role} Dashboard
                </Link>
                <button
                  ...
                  className="block w-full text-left rounded-xl px-4 py-2.5 text-sm font-semibold text-rose-600 hover:bg-rose-50"
                >
                  Logout
                </button>
              </>
